...
function addHexPrefix(addrLike: string) {
...
async function fetchEtherscan(network: string, params: URLSearchParams, logger: ((string) => void)): Promise<any> {
...
export default defineComponent({
  ...
  methods: {
    ...
    async decodeTxHash(epoch: string, hash: string) {
      const transaction: any = (await fetchEtherscan(this.network, new URLSearchParams({
        "module": "proxy",
        "action": "eth_getTransactionByHash",
        "txhash": addHexPrefix(hash),
      }), (x) => this.logs.push(x)))["result"];
